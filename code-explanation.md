...
*   **Live Countdown:** It uses a loop to edit the Telegram message every second, showing a visual progress bar (`█░░░`) and a remaining time countdown.
...
async def live_packet_sniffer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = await context.bot.send_message(chat_id, "🔍 Scanning for BGMI packets...")
    ...
    if output:
        ...
        await message.edit_text(f"✅ Target Found:\n`{output[:50]}`", parse_mode="Markdown")
    else:
        await message.edit_text("❌ No match detected. Ensure you are in a match.")
...
def main_menu_keyboard():
    keyboard = [
        [
            ...
            InlineKeyboardButton("🎯 Live Sniff", callback_data="menu_live")
        ],
        # ... other rows
    ]
